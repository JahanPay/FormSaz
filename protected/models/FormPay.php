...
class FormPay extends CFormModel
{
	...
	function go2bank()
	{
		...
		if(strlen($au)>4)
		{
			$af = Yii::app()->db->createCommand()->update('form_item',array('jahanpay_au'=>$au),'id='.$last);
			if( ! $af)
	...
	function verify($form_item)
	{
		...
		if($bank->verify($form_item->amount,$form_item->id,$form_item->jahanpay_au))
		{
